...
class MyViT(torch.nn.Module):
    def __init__(self, device):
        super().__init__()
        self.model = models.vision_transformer.vit_b_16(weights=models.ViT_B_16_Weights.DEFAULT)
    ...
    def extract_features(self, image):
        ...
        with torch.no_grad():
            feature = self.model._process_input(image)
            batch_class_token = self.model.class_token.expand(image.shape[0], -1, -1)
            feature = torch.cat([batch_class_token, feature], dim=1)
            feature = self.model.encoder(feature)
